...
namespace RegistaShop.WebUI.Services.Concrete
{
	public class ClientCredentialTokenService : IClientCredentialTokenService
	{
		...
		{
			var currentToken = await _clientAccessTokenCache.GetAsync("registashoptoken");
			...
			{
				ClientId = _clientSettings.RegistaShopVisitorClient.ClientId,
				ClientSecret = _clientSettings.RegistaShopVisitorClient.ClientSecret,
				Address = discoveryEndPoint.TokenEndpoint
			};

			var newToken = await _httpClient.RequestClientCredentialsTokenAsync(clientCredentialTokenRequest);
			await _clientAccessTokenCache.SetAsync("registashoptoken", newToken.AccessToken, newToken.ExpiresIn);
