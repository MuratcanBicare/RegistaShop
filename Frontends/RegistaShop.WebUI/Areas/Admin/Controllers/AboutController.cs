...
namespace RegistaShop.WebUI.Areas.Admin.Controllers
{
	...
	public class AboutController : Controller
	{
		...
		public async Task<IActionResult> DeleteAbout(string id)
		{
			...
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.DeleteAsync($"https://localhost:7245/api/Abouts?id={id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index", "About", new { area = "Admin" });
			}
		...
		public async Task<IActionResult> UpdateAbout(UpdateAboutDto updateAboutDto)
		{
			...
			StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
			var responseMessage = await client.PutAsync("https://localhost:7245/api/Abouts/", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index", "About", new { area = "Admin" });
			}
